...
test_deps: $(BUSTED) $(LUV) $(LUAROCKS_DEPS)
...
test_lua: test_deps $(TARGET_DIR)
	@echo Test with $(LUA_VERSION) ......
	@$(HEREROCKS_ACTIVE) && eval $$(luarocks path) && \
	busted --lua=$(TARGET_DIR)/bin/lua  --helper="$(BUSTED_HELPER)" --run=$(BUSTED_TAG) -o htest spec/tests
...
pack:
	$(HEREROCKS_ACTIVE) && eval $$(luarocks path) && \
		luarocks pack ulf.lib-$(ROCKS_PACKAGE_VERSION)-$(ROCKS_PACKAGE_REVISION).rockspec "$(ROCKS_PACKAGE_VERSION)-$(ROCKS_PACKAGE_REVISION)" 

publish:
	$(HEREROCKS_ACTIVE) && eval $$(luarocks path) && \
		luarocks upload ulf.lib-$(ROCKS_PACKAGE_VERSION)-$(ROCKS_PACKAGE_REVISION).rockspec ulf.lib--$(ROCKS_PACKAGE_VERSION)-$(ROCKS_PACKAGE_REVISION).src.rock  --api-key=$(LUAROCKS_API_KEY)
...
$(LUAROCKS_DEPS): $(LUAROCKS) $(BUSTED_HTEST) $(NLUA)
	...
	@$(HEREROCKS_ACTIVE) && eval $$(luarocks path) && \
	luarocks make ulf.lib-$(ROCKS_PACKAGE_VERSION)-$(ROCKS_PACKAGE_REVISION).rockspec && \
	luarocks test --prepare ulf.lib-$(ROCKS_PACKAGE_VERSION)-$(ROCKS_PACKAGE_REVISION).rockspec && \
	touch $(TARGET_DIR)/.deps_installed

test-deps: $(LUAROCKS_DEPS)
